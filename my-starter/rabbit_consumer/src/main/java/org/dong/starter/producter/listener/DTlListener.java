...
@Component
@Slf4j
public class DTlListener {

    @RabbitListener(queues = "testDtlQueue")
    public void receive(Message message, Channel channel) {
        try {
            channel.basicQos(1);
            log.info(message.getMessageProperties().getDeliveryTag()+"--消费者1------"+new String(message.getBody()));
            log.info("处理业务逻辑......");
            int i = 3/0;
...
//            TimeUnit.SECONDS.sleep(2);
            log.info(message.getMessageProperties().getDeliveryTag()+"--消费者2------"+new String(message.getBody()));
            log.info("处理业务逻辑......");
//            int i = 3/0;
            channel.basicAck(message.getMessageProperties().getDeliveryTag(),false);
        }catch (Exception e){
